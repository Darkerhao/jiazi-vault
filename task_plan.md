...
## 当前任务：V1.2 环境变量集与 Windows 验收

- [complete] 核实 Node.js DotEnv 规则与现有加密、锁定、备份调用链。
- [complete] 环境变量条目、导入预览、编辑、单项复制及确认后的完整复制/文件导出。
- [complete] 验证值一致、持久化、回收站、备份恢复、修改主密码与锁定竞态。
- [in_progress] Windows 快捷键冲突实测；Hello 真实取消、真实锁屏/休眠验收与边界记录。
- [in_progress] 生产构建、桌面流程与交付文档更新。

复用 Item/fields 加密协议、项目和环境筛选，不新增依赖或数据库表，不自动改写项目文件。测试使用隔离保险库；不暂存、不提交。签名、硬冷启动、其他平台与 SSH 等仍为后续范围。

本轮工具问题：web 代理 404，改用 PowerShell 读取官方页面；首次文档补丁上下文错误，未产生修改，按真实标题重试。
